...
export default function AuthFlow({ initialMode = "signin" }: AuthFlowProps) {
  ...
  const handleQuickDemoLogin = async () => {
    ...
    setIsLoading(true);
    try {
      const activeEmail = email.trim() || "olamideolanrewaju129@gmail.com";
      const namePart = activeEmail.split("@")[0];
      const derivedName = namePart
        ? namePart.charAt(0).toUpperCase() + namePart.slice(1)
        : "Olamide";
      await authService.quickDemoLogin({
        name: `${derivedName} Olanrewaju`,
        email: activeEmail,
      });
      setSuccessMsg("Signed in successfully! Redirecting to your dashboard...");
  ...
  const handleGoogleLogin = async () => {
    if (!window.google || !(window.google as any).accounts?.id) {
      // Fallback demo Google login if Google script not initialized
      setErrorMsg("");
      setIsLoading(true);
      try {
        await authService.googleLogin("demo-google-credential");
              ...
              {mode === "signin" && (
                <div className="space-y-6 animate-fadeIn">
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full py-3 px-4 bg-teal-50 hover:bg-teal-100 border border-teal-200 text-[#006666] font-bold rounded-xl text-sm flex items-center justify-center gap-2 transition-all cursor-pointer shadow-2xs group"
                  >
              ...
              {mode === "signup" && (
                <div className="space-y-5 animate-fadeIn">
                  ...
                  <button
                    ...
                    onClick={handleQuickDemoLogin}
                    disabled={isLoading}
                    className="w-full py-2.5 px-4 bg-teal-50 hover:bg-teal-100 border border-teal-200 text-[#006666] font-bold rounded-xl text-sm flex items-center justify-center gap-2 transition-all cursor-pointer shadow-2xs group"
                  >
                    <Sparkles className="w-4 h-4 text-teal-600 group-hover:rotate-12 transition-transform" />
                    <span>Instant Demo Sign-In</span>
                  </button>
